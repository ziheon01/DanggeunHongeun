...
router.post('/register', upload.single('proof'), async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'INSERT INTO users (username, email, password, is_verified, proof_url) VALUES (?, ?, ?, ?, ?)',
      [username, email, password, false, proofUrl]
    );

    res.status(201).json({ message: '회원가입 요청 완료. 관리자 승인 대기 중.' });
  } catch (err) {
...
// ✅ 관리자: 승인 대기 중 사용자 목록 조회
router.get('/pending-users', async (req, res) => {
  try {
    const [users] = await pool.query(
      'SELECT id, username, email, proof_url, created_at FROM users WHERE is_verified = false'
    );
    res.json(users);
  } catch (err) {
